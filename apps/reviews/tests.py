...
class TestPlayAssignmentAndPhases(TestCase):
    ...
    def setUpTestData(cls):
        ...
        for i in range(4):
            ...
            CompetitionRole.objects.create(user=user, competition=cls.competition, role='reader')
            cls.readers.append(user)

        cls.play = Play.objects.create(
            competition=cls.competition, title="Test Play", author_email="author@test.com", is_active=True
        )
    ...
    def test_play_excluded_after_two_positive_verdicts(self):
        ...
        submit(Review.objects.get(reader=self.readers[0]), verdict=True, comment="Great")

        assign_play(self.readers[1], self.competition)
        submit(Review.objects.get(reader=self.readers[1]), verdict=True, comment="Awesome")
...
class TestReviewLifecycle(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.competition = Competition.objects.create(title="Lifecycle Comp", date=date(2026, 1, 1))
        ...
        cls.play = Play.objects.create(competition=cls.competition, title="Lifecycle Play", author_email="a@a.com", is_active=True)
        cls.review = Review.objects.create(reader=cls.reader, play=cls.play, phase=Review.Phase.PHASE_1, status=Review.Status.ASSIGNED)
    ...
    def test_save_draft_retains_status(self):
        ...
        url = reverse('reviews:save_draft', kwargs={'competition_slug': self.competition.slug, 'pk': self.review.pk})
    ...
    def test_moderator_can_mark_obsolete(self):
        ...
        url = reverse('reviews:mark_obsolete', kwargs={'competition_slug': self.competition.slug, 'pk': self.review.pk})
